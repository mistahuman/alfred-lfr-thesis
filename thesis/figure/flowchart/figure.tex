...
\begin{document}
 ...
 \tikzstyle{rect} = [draw=black, rectangle, fill=white!20, text width=8em, text centered, minimum height=2em]
 \tikzstyle{elli} = [draw=black, ellipse, fill= white!20, minimum height=2em]
\tikzstyle{circ} = [draw=black, circle, fill=white!20, text width=3em, text centered]
\tikzstyle{diam} = [draw, diamond, fill=white!20, text width=6em, text badly centered, inner sep=0pt]
\tikzstyle{trap} = [draw=black, trapezium, trapezium left angle=70, trapezium right angle=110, fill=white!20, text width=6em, text centered, minimum height=2em]
...
\node (geoco) [rect, rounded corners, left of=geo, xshift=-1cm] {\textbf{GeoCo.c2m} Creazione della geometria complessiva del reattore};
...
\node (setfuelmap) [rect, below of=matex] {SetFuelMap};
\node (setparam) [rect, below of=setfuelmap] {SetParam};
\node (fmap) [circ, below of=setparam] {Fmap};
...
\draw [arrow] (matex) -- (setfuelmap);
\draw [arrow] (setfuelmap) -- (setparam);
\draw [arrow] (setparam) -- (fmap);
